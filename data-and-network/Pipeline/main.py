...
def generate_json_reviews_path(continent: str, country: str, city: str, attraction: str) -> str:
...
def exec_net_build_pipeline():
    ...
    for continent in places_info.continents:
        continent_progress = next((c for c in pipeline_progress.continents if c.name == continent.name), None)
        if continent_progress.progress == '✅':
            logging.info(f"Skipping continent {continent.name} as it is already completed.")
            continue

        for country in continent.countries:
            country_progress = next((c for c in continent_progress.countries if c.name == country.name), None)
            if country_progress and country_progress.progress == '✅':
                logging.info(f"Skipping country {country.name} as it is already completed.")
                continue

            for city in country.cities:
                city_progress = next((c for c in country_progress.cities if c.name == city.name), None)
                if city_progress and city_progress.progress == '✅':
                    logging.info(f"Skipping city {city.name} as it is already completed.")
                    continue

                logging.info(f"Processing city: {city.name} in country: {country.name}, continent: {continent.name}")

            places_api_city_info = places_api.Location(latitude=city.latitude, longitude=city.longitude)
            city_attractions = places_api.getNearbyAttractions(places_api_city_info)

            for attraction in city_attractions:
                attraction_progress = next((a for a in city_progress.attractions if a.id == attraction.id), None)
                if attraction_progress and attraction_progress.progress == '✅':
                    logging.info(f"Skipping attraction {attraction.displayName['text']} as it is already completed.")
                    continue

                reviews = scraper.scrape_google_maps({ "url": f'{attraction.googleMapsUri}&hl=en', "json_path": generate_json_reviews_path(continent.name, country.name, city.name, attraction.displayName['text']) })

                for review in reviews:
                    review_sentences = sentiments.extract_sentences_from_text(review.description['en'])

                    for sentence in review_sentences:
                        adjectives = sentiments.extract_sentence_adjectives(sentence)
                        sentiment_score = sentiments.extract_sentence_sentiment(sentence)
                        weighted_sentiment_score = review.rating * (1 + sentiment_score['compound'])

                        for adjective in adjectives:
                            network.add_edge(city.name, attraction.name, adjective, weighted_sentiment_score)
