...
SAVED_CSV_PATH = os.path.join(MODULE_DIR, "top50_cities_per_top10_countries_per_continent.csv")
SAVED_JSON_PATH = os.path.join(MODULE_DIR, "top50_cities_per_top10_countries_per_continent.json")
...
def get_places(top_countries=10, top_cities=50) -> Places:
    ...
    if os.path.exists(SAVED_JSON_PATH):
        with open(SAVED_JSON_PATH, 'r') as f:
            ...
            logging.info("Loaded saved places (continents, countries and cities) results from previous run.")

            return saved_data

    # Load TSV file (change path to where you extracted cities15000.txt)
    citiesDF = pd.read_csv(CITIES_DATA_PATH, sep="\t", names=citiesCols, header=None, dtype={"population": "Int64"})
    countriesDF = pd.read_csv(COUNTRIES_DATA_PATH, sep="	", names=countriesCols, header=None, dtype={"Population": "Int64"})

    # Sort countries by population and get top 10 per continent
    top_countries = countriesDF[countriesDF["Continent"] != "AN"].sort_values("Population", ascending=False).groupby("Continent").head(top_countries)
    top_cities = {}
    for _, row in top_countries.iterrows():
        country_code = row["ISO"]
        top_cities[country_code] = citiesDF[citiesDF["country_code"] == country_code].sort_values("population", ascending=False).head(top_cities)
    ...
    for country_code, df in top_cities.items():
        country_name = countriesDF[countriesDF["ISO"] == country_code]["Country"].values[0]
    ...
    combined_top_cities = combined_top_cities[["name", "country", "population", "latitude", "longitude", "continent"]]

    # Get the Places dataclass structure
    places = Places(continents=[])
    for continents in combined_top_cities["continent"].unique():
        continent_countries = []
        continent_df = combined_top_cities[combined_top_cities["continent"] == continents]
    ...
    if os.path.exists(SAVED_JSON_PATH):
        logging.info("Saving places (continents, countries and cities) results from current run.")
