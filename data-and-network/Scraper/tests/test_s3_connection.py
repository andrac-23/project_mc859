...
class TestS3Connection:
    ...
    def test_s3_connection_when_enabled(self, use_s3, s3_config):
        ...
        try:
            ...
            pytest.fail(
                'AWS credentials not found. Check your configuration or environment.'
            )
            ...
            if error_code == '404':
                ...
                pytest.fail(
                    f"Access denied to S3 bucket '{bucket_name}'. Check your credentials and permissions."
                )
            else:
    ...
    def test_s3_handler_initialization(self, config):
        ...
        try:
            ...
            if expected_enabled:
                ...
                if bucket_name:
                    assert s3_handler.bucket_name == bucket_name, (
                        'S3Handler should load bucket name from config'
                    )
                else:
                    # If no bucket name, handler should be disabled
                    assert not s3_handler.enabled, (
                        'S3Handler should be disabled when bucket_name is missing'
                    )
